...
class MCGRAN(nn.Module):
  """ Graph Recurrent Attention Networks """

  def __init__(self, config):
    super(MCGRAN, self).__init__()
    self.config = config
...
#    ixs = torch.arange(N_max, dtype=torch.int64).to(node_feat_edge.device)
#    zeroed = torch.where(ixs[None, :] == 1, torch.tensor(0.), t)

    node_state = node_feat_label.reshape(B * C, N_max, self.hidden_dim)
...
#    node_state = node_state + beta_condition
  ...
  def _sampling(self, B, conditional_data, expanded_search=False):
    ...
    with torch.no_grad():       
        ...
        node_state = node_feat.reshape(B, N, self.hidden_dim)
...
def get_node_idx_feat(node_idx, max_num_nodes, batch_size):
    ...
    for ii in range(batch_size):
        for jj in range(0, max_num_nodes):
            if jj == 0:
                node_idx_feat += [np.ones(K) * np.inf]
            else:
                node_idx_feat += [
                    np.concatenate([np.arange(jj) + ii * max_num_nodes,
                                   np.ones(K) * np.inf]) 
                ]
    generated_node_idx_feature = np.concatenate([[node_idx_feat[node_idx]] 
                                for i in range(batch_size)])
    
    generated_node_idx_feature = np.concatenate(
                [
                        bb + ii * max_num_nodes
                        for ii, bb in enumerate(generated_node_idx_feature)
                ],
                axis=0) + 1
    
    generated_node_idx_feature[np.isinf(generated_node_idx_feature)] = 0
    generated_node_idx_feature = generated_node_idx_feature.astype(np.int64)
    
    return generated_node_idx_feature
